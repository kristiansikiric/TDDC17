...
public class CustomGraphSearch implements SearchObject {
	...
	public ArrayList<SearchNode> search(Problem p) {
		...
		while(!frontier.isEmpty()){
			currNode = frontier.removeFirst();
			if(p.isGoalState(currNode.getState())){
				path = currNode.getPathFromRoot();
				return path;
			}
			ArrayList<GridPos> childStates = p.getReachableStatesFrom(currNode.getState());
			for (GridPos childs : childStates){
				SearchNode child = new SearchNode(childs,currNode);
				if(!frontier.contains(child) && !explored.contains(child)){
					if(insertFront){
						frontier.addNodeToFront(child); //LIFO
					}
					else{
						frontier.addNodeToBack(child); //FIFO
					}
				}
			}
